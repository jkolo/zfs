...
zfs_ioc_dataset_list_bulk(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	while (count < max_count) {
		...
		if (!simple) {
			/* Add full properties when not in simple mode */
			nvlist_t *props = NULL;
			if (dsl_prop_get_all(child_os, &props) == 0) {
				dmu_objset_stats(child_os, props);
				fnvlist_add_nvlist(ds_props, "props", props);
				nvlist_free(props);
			}
		}
...
zfs_ioc_snapshot_list_bulk(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	while (count < max_count) {
		...
		if (!simple) {
			/* Add full properties - need to open objset */
			objset_t *snap_os;
			if (dmu_objset_from_ds(snap_ds, &snap_os) == 0) {
				nvlist_t *props = NULL;
				if (dsl_prop_get_all(snap_os, &props) == 0) {
					dmu_objset_stats(snap_os, props);
					fnvlist_add_nvlist(snap_props,
					    "props", props);
					nvlist_free(props);
				}
			}
